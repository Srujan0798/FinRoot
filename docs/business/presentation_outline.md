...
- **FRB results:** FinRoot's reasoning pipeline scores significantly higher than a naive RAG baseline on reasoning quality. *(Numbers from `results/metrics.json` at `as_of_sha = 75eca43`. Regenerate via `make evals`.)*
