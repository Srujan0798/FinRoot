...
2. **Numbers drift across docs.**
   The current `results/metrics.json` (as_of_sha `e955fb9`, regenerated 2026-07-24) reports
   **83 tasks, 11 domains, +158.30% composite lift vs RAG, FinRoot mean 0.8741**. The deck uses
   these numbers. Earlier waves had `as_of_sha = e955fb9` (FinRoot 0.795, RAG 0.334, +137.8%);
   wave-13 reconciled all `*.md` files to the current `e955fb9` numbers. If a future wave
